...
if (select.source == "ms") {
        ...
        MS_Download <- function(Stock, Type, Period) {
                ...
                type <-
                        ifelse(
                                Type == "IS",  "income-statement/is.html?t=", "balance-sheet/bs.html?t="
                        )
                if (Type == "CF")
                        type <- "cash-flow/cf.html?t="
                period <-
                        ifelse(Period == "yr",  "Annual", "Quarterly")
                fileURL <- paste0(fileURL, type, Stock, endURL)
                ms.env$mybrowser$navigate(fileURL)
                Sys.sleep(2)
                
                msTimeframe <-
                        ms.env$mybrowser$findElement(using = 'css selector', "#Li1")
                msTimeframe$clickElement()
                msSpan <-
                        ms.env$mybrowser$findElement(using = 'class', "prem1")
                msSpan$clickElement()
                Sys.sleep(2)
                
                msPeriod <-
                        ms.env$mybrowser$findElement(using = 'css selector', "#menu_A")
                msPeriod$clickElement()
                msPeriodli <-
                        ms.env$mybrowser$findElement(using = 'li', period)
                msPeriodli$clickElement()
                Sys.sleep(2)
                
                msbutton <-
                        ms.env$mybrowser$findElement(using = "class", "rf_export")
        ...
        MS_Login <- function(password) {
                loginURL <-
                        "https://www.morningstar.com/members/login.html?vurl="
                ms.env$mybrowser$navigate(loginURL)
                
                msEmail <-
                        ms.env$mybrowser$findElement(using = 'css selector', "#uEmail")
                msEmail$sendKeysToElement(list("ted@powersdata.com"))
                msP <-
                        ms.env$mybrowser$findElement(using = 'css selector', "#uPassword")
                msP$sendKeysToElement(list(password))
                msbutton <-
                        ms.env$mybrowser$findElement(using = "xpath", "//*/input[@ng-click = 'submit(user)']")
                msbutton$clickElement()
        }
        MS_Web_Setup <- function() {
                ...
                assign(
                        "fprof", getFirefoxProfile(
                                "C:/Users/Ted/AppData/Roaming/Mozilla/Firefox/Profiles/wlb50pz5.WebDriver_Profile", useBase = TRUE
                        ), envir = as.environment(ms.env)
                )
                
                assign(
                        "mybrowser", remoteDriver(extraCapabilities = ms.env$fprof), envir = as.environment(ms.env)
                )
